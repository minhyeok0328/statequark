...
def test_smart_thermostat_system():
    ...
    efficiency = quark(energy_efficiency, deps=[current_temp, outdoor_temp, heating_system])
    ...
    current_temp.set_sync(19.5)  # Lower than occupied target but within unoccupied range
...
async def test_async_sensor_network():
    ...
    def network_callback(q):
        status = q.value
        network_events.append(f"Network: {status['online_sensors']} sensors, "
